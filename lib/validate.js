...
function validateFormData(formData, schema, customValidate, transformErrors) {
  ...
  var getResultForErrorHandler = function getResultForErrorHandler(errorHandler) {
  ...
  if (customValidate) {
    var errorResponse = customValidate(formData, createErrorHandler(formData)); // errorResponse is either the result or a promise that resolves the result.

    return errorResponse.then ? errorResponse.then(function (errorHandler) {
      return getResultForErrorHandler(errorHandler);
