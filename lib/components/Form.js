...
function (_Component) {
  ...
  (0, _createClass2["default"])(Form, [{
    ...
    value: function getStateFromProps(props, inputFormData) {
      ...
      var additionalMetaSchemas = props.additionalMetaSchemas; //const validationHandledExternally = props.noValidate || props.extraErrors === state.extraErrors;

      var _ref = mustValidate ? this.validate(formData, schema, additionalMetaSchemas, customFormats) : {
        errors: !props.noValidate && state.errors || [],
        errorSchema: !props.noValidate && state.errorSchema || {} //errors: (validationHandledExternally && state.errors) || [],
        // errorSchema: (validationHandledExternally && state.errorSchema) || {},

      },
          errors = _ref.errors,
          errorSchema = _ref.errorSchema; //console.log("result", mustValidate, props.noValidate, props.extraErrors && mergeObjects(errorSchema, props.extraErrors), props.extraErrors, errorSchema);
