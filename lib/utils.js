...
function computeDefaults(schema, parentDefaults, formData, definitions) {
  if ("$ref" in schema) {
    var refParentDefaults = parentDefaults !== undefined ? parentDefaults : schema.default;
    return computeDefaults(retrieveSchema(schema, definitions, formData), refParentDefaults, formData, definitions);
  } else {
    switch (schema.type) {
      case "array":
        {
          var minItemsLength = schema.minItems || 0;
          var length = minItemsLength && isMultiSelect(schema, definitions) ? 0 : Array.isArray(formData) ? Math.max(formData.length, minItemsLength) : Math.max((schema.default || []).length, (parentDefaults || []).length, minItemsLength, Array.isArray(schema.items) ? schema.items.length : 0);
          return new Array(length).fill(undefined).map(function (_, i) {
            ...
            } else if (Array.isArray(schema.items) && schema.items.length > i && schema.items[i] && "default" in schema.items[i]) {
              defaultItem = schema.items[i].default;
            }

            var childFormData = formData && formData.length > i ? formData[i] : undefined;

            return computeDefaults(childSchema, defaultItem, childFormData, definitions);
          });
        }
      case "object":
        {
          var defaultFilled = Object.keys(schema.properties || {}).reduce(function (acc, key) {
            var propSchema = schema.properties[key];

            var propParentDefault = undefined;
            ...
            var propFormData = formData && key in formData ? formData[key] : undefined;

            acc[key] = computeDefaults(propSchema, propParentDefault, propFormData, definitions);
            return acc;
          }, {});
